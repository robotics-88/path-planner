...
class planner {
  public:
    ...
    void setStart(double x, double y, double z, double vx, double vy, double vz,
    ...
    void setlocalpose(geometry_msgs::msg::PoseStamped pose) { local_pose = pose; }
    ...
    // Constructor
    planner(void) { path_size_int64.data = 0; }
    // Destructor
    ~planner() {}

    void replan(void) {
        if (sqrt(pow((goal_pt(0) - start_pt(0)), 2) + pow((goal_pt(1) - start_pt(1)), 2) +
                 pow((goal_pt(2) - start_pt(2)), 2)) < 0.3) {
            return;
        }
        if (path_size_int64.data != 0 && set_start) {
            ...
            for (std::size_t idx = 0; idx < path_size_int64.data; idx = idx + 40) {
                if (!replan_flag) {
                    ...
                    replan_flag =
                        !kino_path_finder_->isSafe(kino_nav_path.poses[idx].pose.position.x,
                                                   kino_nav_path.poses[idx].pose.position.y,
                                                   kino_nav_path.poses[idx].pose.position.z);
    ...
    bool plan(void) {
        ...
        if (firstplan_flag == true ||
            replan_time_index == -1) // || (replan_time_index==(path_size_int64.data-1))
        {
            ...
            int status =
                kino_path_finder_->search(start_pt, start_vel, start_acc, goal_pt, goal_vel, true);

            if (status == KinodynamicAstar::NO_PATH) {
                // retry searching with discontinuous initial state
                kino_path_finder_->reset();
                status = kino_path_finder_->search(start_pt, start_vel, start_acc, goal_pt,
                                                   goal_vel, false);
            ...
            if (used_time_index + vel_horizon >= path_size_int64.data - 1) {
                replan_startpt_vel(0) =
                    (kino_nav_path.poses[used_time_index].pose.position.x -
                     kino_nav_path.poses[used_time_index - vel_horizon].pose.position.x) /
                    (0.01 * vel_horizon);
                replan_startpt_vel(1) =
                    (kino_nav_path.poses[used_time_index].pose.position.y -
                     kino_nav_path.poses[used_time_index - vel_horizon].pose.position.y) /
                    (0.01 * vel_horizon);
                replan_startpt_vel(2) =
                    (kino_nav_path.poses[used_time_index].pose.position.z -
                     kino_nav_path.poses[used_time_index - vel_horizon].pose.position.z) /
                    (0.01 * vel_horizon);
            ...
            // RCLCPP_INFO("StartPOINT IS
            // %f,%f,%f",replan_startpt(0),replan_startpt(1),replan_startpt(2)); RCLCPP_INFO("Before
            // KINODYNAMIC SEARCH TIME: %f",(node->get_clock()->now() - t1).toSec());

            int status = kino_path_finder_->search(replan_startpt, replan_startpt_vel, start_acc,
                                                   goal_pt, goal_vel, true);

            if (status == KinodynamicAstar::NO_PATH) {

                // retry searching with discontinuous initial state
                kino_path_finder_->reset();
                status = kino_path_finder_->search(replan_startpt, replan_startpt_vel, start_acc,
                                                   goal_pt, goal_vel, false);
        ...
        // RCLCPP_INFO("KINODYNAMIC SEARCH TIME: %f,search_count = %d,search average time =
        // %f",t_search,kino_path_finder_->search_count,kino_path_finder_->search_time_amount/kino_path_finder_->search_count);
        ...
        Eigen::Quaterniond quat_px4(local_pose.pose.orientation.w, local_pose.pose.orientation.x,
                                    local_pose.pose.orientation.y, local_pose.pose.orientation.z);
        ...
        T_odom << uav_odom.pose.pose.position.x, uav_odom.pose.pose.position.y,
            uav_odom.pose.pose.position.z;
        T_local << local_pose.pose.position.x, local_pose.pose.position.y,
        ...
        for (int i = 0; i < kino_path.size(); i++) {
            ...
            // RCLCPP_INFO("i = %d,pos =
            // %f,%f,%f",i,kino_path[i](0),kino_path[i](1),kino_path[i](2));
...
// This is gross formatting but this thing was built gross-ly and this works. TODO fix all the bad
// code practice here
...
void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    // {
    // if(fabs(cloud_input.points[i].x - cur_pos(0))>20 || fabs(cloud_input.points[i].y -
    // cur_pos(1))>20 || fabs(cloud_input.points[i].z - cur_pos(2))>20)
    // {
    // 	continue;
    // }else{
...
void poseCb(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    ...
    planner_ptr->setStart(msg->pose.position.x, msg->pose.position.y, msg->pose.position.z, 0, 0, 0,
                          uav_odometry);
    planner_ptr->init_start();
...
int main(int argc, char **argv) {
    ...
    auto odom_sub =
        node->create_subscription<nav_msgs::msg::Odometry>("/mavros/odometry/out", 1, odomCb);
    auto pointcloud_sub =
    ...
    hb_qos.liveliness();
    heartbeat_pub =
        node->create_publisher<std_msgs::msg::Header>("/path_planner/heartbeat", hb_qos);
    vis_pub = node->create_publisher<visualization_msgs::msg::Marker>("visualization_marker", 0);
    traj_pub =
        node->create_publisher<trajectory_msgs::msg::MultiDOFJointTrajectory>("waypoints", 1);
    ...
    Bspline_pub = node->create_publisher<trajectory_msgs::msg::MultiDOFJointTrajectory>(
        "smooth_waypoints", 1);
